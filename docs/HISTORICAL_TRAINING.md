...
以下保留仓库原 README 的实验记录，用于理解原作者的训练路线。
提到的 `dataset.jsonl`、checkpoint、GPU 吞吐量及问答输出没有随仓库发布，本次未验证。
路径和命令反映历史实验，不是一套在新克隆中可直接运行的教程。
原验证集按连续 token 切分，且课程样本重复，历史验证指标可能受到数据泄漏影响。
代码完整性检查不能替代语言能力评测；最新可执行说明见根目录 README。
旧命令如需 GPU，请显式增加 `--device cuda`（新版 CLI 默认 CPU）。
